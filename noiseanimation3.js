...
void main () {
  ...
  float mask2 = aastep(0.25 + sin(time + vUv.x * 0.1), d);

  vec3 fragColor = mix(color, vec3(1.0), mask2);
...
console.log(points);
  ...
  const moonMaterial = new THREE.ShaderMaterial({ //meshnormalmaterial is good for debugging, mesh basic material can add texture/image, meshstandardmaterial can have lights
    ...
    uniforms: {
      points: { value: points },
      time: { value: 0 },
      color: { value: new THREE.Color("#283")}
    },
    vertexShader,
    fragmentShader
  });
  ...
  moonMesh.scale.setScalar(0.5);    mesh.scale.setScalar(Random.range(0.25, 1) * 0.25);    //0.25 /Random.gaussian(0.25, 1) * 0.25); Random.range(0.25, 1) * 0.25);
  moonGroup.scale.setScalar(Random.range(0.25, 1) * 0.25);    //0.25 /Random.gaussian(0.25, 1) * 0.25); Random.range(0.25, 1) * 0.25);
  scene.add(moonMesh);
  scene.add(moonGroup);
